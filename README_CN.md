...
### ⏰ 定时发推文（全新功能！）
- ✅ **定时发布单条推文** - 在指定时间发布推文
- ✅ **定时发布推文串** - 在指定时间发布完整的推文串
- ✅ **循环定时推文** - 按固定间隔循环发布推文
- ✅ **灵活的时间格式** - 支持绝对时间（2024-01-15T14:30:00）和相对时间（+10m, +2h, +1d）
- ✅ **后台调度器** - 自动执行，无需手动干预
- ✅ **调度管理** - 查看、取消和管理所有定时推文
- ✅ **智能间隔** - 完美适用于定期内容发布和营销活动
...
### 📈 趋势与话题
- ✅ 获取全球趋势话题 *（需要付费API计划）*
- ✅ 获取地区趋势话题（支持多个国家和地区）*（需要付费API计划）*
- ✅ 获取可用趋势位置列表 *（需要付费API计划）*
- ✅ 获取特定话题的详细信息和相关推文 *（免费账户有限制）*
- ✅ 搜索与关键词相关的热门标签 *（免费账户有限制）*
- ✅ 话题参与度分析和统计
...
当推文发布失败时，您可以选择是否自动删除草稿：

- **启用自动删除**（默认）：发布失败时自动删除草稿，避免累积无效草稿
- **禁用自动删除**：发布失败时保留草稿，允许手动重试或修改

#### 配置方法

1. **通过环境变量**：在配置文件中添加 `"AUTO_DELETE_FAILED_DRAFTS": "true"` 或 `"false"`
2. **通过命令**：使用 "启用发布失败时自动删除草稿功能" 或 "禁用发布失败时自动删除草稿功能"
3. **查看状态**：使用 "查看当前自动删除草稿的配置状态"
...
### 趋势分析功能
- **全球趋势监控** - 实时获取全球热门话题和趋势
- **地区趋势分析** - 支持美国、日本、英国等多个国家和地区
- **话题深度分析** - 获取特定话题的相关推文、参与度统计
- **标签趋势发现** - 基于关键词搜索相关的热门标签
- **趋势数据统计** - 提供推文量、参与度、平均互动等指标
...
### 趋势功能限制
**免费Twitter API账户无法使用以下功能：**
- ❌ 获取全球趋势话题
- ❌ 获取地区趋势话题  
- ❌ 获取可用趋势位置列表

**免费账户可以使用（有严格限制）：**
- ✅ 话题详细分析 - 仅最近7天推文，月调用次数很少
- ✅ 热门标签搜索 - 仅最近7天推文，月调用次数很少

### API计划建议
- **学习测试**: 免费计划足够基本功能使用
- **个人项目**: 建议升级到Basic计划 ($100/月)
- **商业应用**: 建议使用Pro计划 ($5000/月)
...
- **[趋势功能使用指南](趋势功能使用指南.md)** - 🆕 完整的趋势与话题功能指南
